...
- Wrap both tools in a tiny `httpx` client with a `CENSUS_API_KEY` env var
- Cache responses in-memory per session with `functools.lru_cache` — the Census API is slow enough that this noticeably improves demo feel
...
└── src/
    ...
    │   ├── neighborhood.py    # Census API tools + fetch_neighborhood_raw helper
...
- Register for OpenAI and Census API keys
...
- Implement the two Census tools (`get_demographics`, `get_housing_stats`) plus the `fetch_neighborhood_raw` helper
- Implement the two schools tools (`search_schools_near`, `get_school_stats`) plus the `fetch_schools_summary` helper
- Use `functools.lru_cache` on the Census fetcher for per-session memoization

**Deliverable:** Every tool verified against real data (Census key required).
